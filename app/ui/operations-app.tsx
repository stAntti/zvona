...
const mandatoryQuestions = ['Есть ли сейчас онлайн-касса и каким решением пользуетесь?', 'Что в текущем решении неудобно или что нужно для новой точки?', 'Кто принимает решение и когда удобно обсудить условия?']
...
function OperatorCall({ account, onBack, onComplete }: { account: Account; onBack: () => void; onComplete: (result: Omit<CallResult, 'accountId' | 'quality' | 'payout' | 'completedAt'>) => void }) {
  ...
  const transcript = useMemo(() => [
    ['Оператор', 'Здравствуйте! Подскажите, у вас уже есть онлайн-касса или сейчас выбираете решение для точки?'],
    ['Клиент', 'Есть, работаем с другим сервисом. А что вы предлагаете?'],
    ['Оператор', 'Помогаем подобрать и запустить онлайн-кассу. Сначала бесплатно сравним условия с вашим текущим решением — можно уточнить, что сейчас неудобно?'],
    ['Клиент', 'Нас не устраивает поддержка. Можете посчитать другой вариант?'],
  ], [])
  ...
  return <><button className="back-button" onClick={onBack}><ArrowLeft size={16}/> Вернуться в очередь</button><section className="call-header"><div><span className="live-indicator"><i/> Демонстрационный звонок</span><h1>{account.name}</h1><p>{contact?.value ?? 'Контакт ещё не найден · реальная связь не устанавливается'}</p></div><div className="call-timer"><Phone size={16}/><strong>02:34</strong></div></section><div className="operator-layout"><section className="call-main"><article className="call-company"><div className="company-monogram small">{account.name.slice(0, 2).toUpperCase()}</div><div><span>Краткое AI-досье</span><p>{account.aiSummary}</p></div></article><article className="conversation-goal"><span>Цель разговора</span><strong>Понять, нужна ли касса или есть ли причина рассмотреть замену, и договориться о следующем шаге</strong></article><article className="say-now"><span><Sparkles size={16}/> Скажите сейчас</span><p>«Если касса уже есть — что в работе с ней хотелось бы улучшить? Мы можем подготовить расчёт без обязательств.»</p></article><article className="transcript"><div className="section-row"><div><span>Разговор</span><h2>Демонстрационный транскрипт</h2></div><small>появляется по ходу звонка</small></div>{transcript.map(([speaker, text], index) => <div className={`transcript-line ${speaker === 'Оператор' ? 'operator' : ''}`} key={index}><span>{speaker}</span><p>{text}</p></div>)}</article></section><aside className="call-assistant"><section><h2>Обязательные вопросы</h2>{mandatoryQuestions.map((question, index) => <label className="call-check" key={question}><input type="checkbox" checked={askedQuestions[index]} onChange={event => toggleQuestion(index, event.target.checked)}/><span>{question}</span></label>)}</section><section className="objection"><span>Типовое возражение</span><strong>«У нас уже есть касса»</strong><p>Разрешённый ответ: «Понимаю. Не будем спорить с вашим решением — подскажите, всё ли устраивает в поддержке, условиях и работе кассы?»</p></section><section className="call-result"><h2>Результат разговора</h2><label>Итог<select value={outcome} onChange={event => changeOutcome(event.target.value)}><option>Интерес есть</option><option>Перезвонить позже</option><option>Неверный контакт</option><option>Неактуально</option></select></label><label className="evidence-check"><input type="checkbox" checked={decisionMaker} onChange={event => setDecisionMaker(event.target.checked)}/> Найден ответственный</label><label className="evidence-check"><input type="checkbox" checked={activeNeed} onChange={event => setActiveNeed(event.target.checked)}/> Есть актуальная потребность</label><label className="evidence-check"><input type="checkbox" checked={agreedNextStep} onChange={event => setAgreedNextStep(event.target.checked)}/> Согласован следующий шаг</label><label>Заметки<textarea value={notes} onChange={event => setNotes(event.target.value)} placeholder="Например: использует конкурента, не устраивает поддержка, согласен на расчёт"/></label><button className="button primary wide" onClick={() => onComplete({ outcome, notes, decisionMaker, activeNeed, agreedNextStep, askedQuestions })}>Завершить и получить разбор <ArrowRight size={16}/></button></section></aside></div></>
}
...
function QaView({ result, account, next, onNext, onDashboard, onResults }: { result: CallResult; account: Account; next?: Account; onNext: (id: string) => void; onDashboard: () => void; onResults: () => void }) {
  ...
  const nextStep = result.outcome === 'Неверный контакт' ? 'Вернуть компанию на поиск правильного контакта' : result.outcome === 'Неактуально' ? 'Исключить компанию из текущей кампании' : result.decisionMaker && result.activeNeed && result.agreedNextStep ? 'Передать лид менеджеру и подготовить расчёт условий по онлайн-кассе' : 'Уточнить недостающие данные'
